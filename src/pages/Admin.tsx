...
// استخدام مسارات مباشرة لضمان العثور على الملفات وتخطي أخطاء Vercel
import { useAuth } from '../contexts/AuthContext'; 
import { db } from '../lib/firebase'; 
import { collection, addDoc, getDocs, deleteDoc, doc } from 'firebase/firestore';

// تم وضع المفتاح الخاص بك بشكل صحيح داخل علامات الاقتباس
...
export default function Admin() {
  ...
  const handleImageUpload = async (e: any) => {
    ...
    } catch (err) { alert("خطأ في الرفع"); } finally { setLoading(false); }
  };

  const handleSave = async () => {
    if (!name || !price || !imageUrl) return alert("أكمل البيانات");
    try {
  ...
  return (
    <div className="min-h-screen bg-[#0F111A] text-white p-6" dir="rtl">
      <div className="max-w-4xl mx-auto pt-20">
        ...
        <button onClick={() => setShowProductDialog(true)} className="w-full bg-[#007AFF] py-4 rounded-xl font-bold mb-8">+ إضافة منتج جديد</button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {products.map(p => (
            <div key={p.id} className="bg-[#1A1D29] rounded-2xl overflow-hidden border border-white/5 p-4">
              ...
              <button onClick={async () => { if(confirm("حذف؟")) { await deleteDoc(doc(db, "products", p.id)); fetchProducts(); } }} className="text-red-400 flex items-center gap-2 text-sm"><Trash2 size={16}/> حذف</button>
            </div>
          ))}
        </div>
      </div>
      ...
      {showProductDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
          <div className="bg-[#1A1D29] w-full max-w-md rounded-2xl p-6 border border-white/10">
            <h2 className="text-xl font-bold mb-6 text-center">إضافة هاتف</h2>
            <div className="space-y-4">
              <div onClick={() => document.getElementById('imgInp')?.click()} className="border-2 border-dashed border-white/10 rounded-xl h-44 flex items-center justify-center cursor-pointer bg-white/5 overflow-hidden relative">
                {imagePreview ? <img src={imagePreview} className="w-full h-full object-cover" alt="" /> : <ImageIcon className="opacity-20 w-10 h-10" />}
                {loading && <div className="absolute inset-0 bg-black/60 flex items-center justify-center">جاري الرفع...</div>}
              </div>
              ...
              <button onClick={handleSave} className="w-full bg-[#007AFF] text-white py-3 rounded-lg font-bold">نشر المنتج الآن</button>
              <button onClick={() => setShowProductDialog(false)} className="w-full text-white/40 text-sm">إلغاء</button>
            </div>
          </div>
        </div>
      )}
